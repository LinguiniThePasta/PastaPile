    ...
    return (
        <>
            <div className="bg-gray-100 relative flex justify-between items-stretch overflow-hidden min-h-[500px] max-h-[700px]">
                <a href={article.url} className="absolute flex flex-col pl-32 justify-center bg-gray-100 break-words w-2/4 hover:w-3/4 transition-all delay-150 duration-200 ease-in-out p-4 z-10 h-full">
                    <span className={"text-5xl"}>{article.title}</span>
                    <span className={"text-lg py-5 text-gray-500"}>{TruncateText(article.body, 350)}...</span>
